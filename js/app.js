...
function cardThumbContent(card){
  const userImg = Store.getCardImage(card.id);
  const symbol = card.arcana==="major" ? "✦" : ({wands:"🔥",cups:"💧",swords:"🌬",pentacles:"⛰"}[card.suit]||"✦");
  if(userImg) return `<img src="${userImg}" alt="${card.name_cn}" onerror="this.outerHTML='${symbol}'">`;
  const localPath = (typeof localImagePath === "function") ? localImagePath(card) : null;
  if(localPath) return `<img src="${localPath}" alt="${card.name_cn}" onerror="this.outerHTML='${symbol}'">`;
  return symbol;
}
...
function renderDrawCanvas(){
  ...
  canvas.innerHTML = spread.positions.map((p,i)=>{
    ...
    const cardVisual = card
      ? `<div class="card-face">${cardThumbContent(card)}</div><div class="slot-card-name">${card.name_cn}${slot.reversed?'<span class="reversed-tag">（逆位）</span>':''}</div>`
      : (i+1);
    return `
